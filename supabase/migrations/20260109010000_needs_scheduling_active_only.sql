-- Update get_clients_needs_scheduling to only include active clients (not pending)
-- Pending clients are holding slots but may not need scheduling yet
...
BEGIN
  ...
  WHERE c.user_id = auth.uid()
    -- Only include active clients (not pending, waiting, or inactive)
    AND (
      c.status = 'active'
