-- Allow client users (linked via auth_user_id) to see their own session history
...
BEGIN
  ...
  WHERE sn.client_id = p_client_id
    AND (
      sn.user_id IN (SELECT accessible_user_ids())
      OR sn.client_id = (SELECT get_portal_client_id())
    )
