-- Migration: Fix populate_sessions_from_calendar_events function
-- File: supabase/migrations/20250630050000_fix_populate_sessions_function.sql

-- Drop the existing function
DROP FUNCTION IF EXISTS populate_sessions_from_calendar_events();

-- Recreate the function without the non-existent status column
...
BEGIN
    ...
    LOOP
        BEGIN
            -- Detect session status based on calendar event patterns (no status column in calendar_events)
...
-- Also fix the sync_sessions_for_period function
DROP FUNCTION IF EXISTS sync_sessions_for_period(TIMESTAMPTZ, TIMESTAMPTZ);
...
'Syncs sessions for a specific time period from calendar_events with intelligent status detection.'; 