...
export function SourceBadge({ source }: { source: string | null }) {
  if (source === 'defacto') {
    return (
      <Badge variant="outline" className="text-2xs px-1.5 py-0 border-chart-3/30 text-chart-3 font-medium">
        Defacto
      </Badge>
    )
  }
  if (source === 'session') {
    return (
      <Badge variant="outline" className="text-2xs px-1.5 py-0 border-primary/30 text-primary font-medium">
        Session
      </Badge>
    )
  }
  if (source === 'granola') {
    return (
      <Badge variant="outline" className="text-2xs px-1.5 py-0 border-success/30 text-success font-medium">
        Granola
      </Badge>
    )
  }
    ...
    <>
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <span
            className={cn(
              'action-row-title text-base leading-[1.15]',
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          {isResolved && action.status === 'cancelled' && (
            <span className="text-2xs font-semibold uppercase tracking-wider text-muted-foreground">Cancelled</span>
          )}
