    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="border-b border-border/50 sticky top-0 bg-background z-10">
        <div className="px-6 py-3 flex items-center justify-between">
          <button
            onClick={onBack}
            className="flex items-center gap-1.5 text-ui-lg text-muted-foreground hover:text-foreground transition-colors"
          >
            <ArrowLeft className="h-4 w-4" />
            Back
          </button>
          <div className="text-center">
            <p className="text-ui-lg font-medium text-foreground">{clientName}</p>
            {subtitle && (
              <p className="text-ui-base text-muted-foreground">{subtitle}</p>
            )}
          </div>
          <div className="flex items-center gap-2">
            ...
            <span className={`text-ui-xs flex items-center gap-1 ${saveStatus === 'error' ? 'text-destructive font-medium' : 'text-muted-foreground/60'}`}>
      ...
      <div className="w-2xl mx-auto px-6 py-8 pb-[50vh] space-y-10 max-sm:w-full max-sm:px-4">
        {/* Connection */}
        <section>
          <h2 className="text-ui-base font-medium text-muted-foreground uppercase tracking-widest mb-4">
            Connection
          </h2>
          {dataLoaded ? (
            <SessionEditor
