...
export function ActionReviewSection({ clientId, sessionNoteId, onActionToggled }: ActionReviewSectionProps) {
  ...
  if (loading) {
    return <p className="text-ui-lg text-muted-foreground">Loading actions...</p>
  }

  if (actions.length === 0 && completedActions.length === 0 && !showCompleted) {
    return <p className="text-ui-lg text-muted-foreground">No open actions</p>
  }
  ...
  return (
    <div className="space-y-1.5">
      ...
      <button
        ...
        className="flex items-center gap-1 text-ui-sm text-muted-foreground hover:text-foreground transition-colors cursor-pointer pt-1"
      >
      ...
      {showCompleted && completedActions.length === 0 && (
        <p className="text-ui-sm text-muted-foreground pl-4">No actions completed this week</p>
      )}
    </div>
  )
}
