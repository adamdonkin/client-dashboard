...
export function SessionSummary({ sessionNoteId, actionsModified }: SessionSummaryProps) {
  ...
  if (loading) {
    return <p className="text-[15px] text-muted-foreground">Loading summary...</p>
  }
          ...
          <div className="space-y-1">
            {createdActions.map(action => (
              <div key={action.id} className="flex items-center gap-3 py-1.5 px-3 rounded bg-muted/30">
                <span className="text-[15px] flex-1">{action.title}</span>
                {action.due_date && (
        ...
        <div>
          ...
          <div className="space-y-1">
            {completedActions.map(action => (
              <div key={action.id} className="flex items-center gap-3 py-1.5 px-3 rounded bg-muted/30">
                <span className="text-[15px] flex-1 line-through text-muted-foreground">{action.title}</span>
              </div>
            ))}
          </div>
        </div>
